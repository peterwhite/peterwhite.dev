...
With PlanetScale, branching isn't just for your code – it's for your database schemas too. This innovative approach lets you test schema changes before applying them to your production database, ensuring a seamless and error-free transition. Just as developers create branches in their version control system to make changes to their code, PlanetScale enables branching for database schemas, merging the worlds of code and data.

This branching ability doesn't just make life easier for developers; it also creates a safety net. Developers can work with their own pristine remote databases, free from the fear of impacting their colleagues' work. It's like having your own sandbox, but with the power and convenience of a full-fledged database.
...
This may have a pricing impact, as PlanetScale's pricing is based on the number of rows reads. So, where the foreign key have created an index before, you should manually add an index, otherwise PlanetScale will have to scan the entire table to find the row you're looking for. Thankfully, Prisma automatically suggests indexes to add to your schema with their VSCode extension.
